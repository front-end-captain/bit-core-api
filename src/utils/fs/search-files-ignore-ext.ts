...
export function searchFilesIgnoreExt(files: Vinyl[], fileName: PathOsBased): Vinyl | null;
export function searchFilesIgnoreExt(
  files: Vinyl[],
...
export function searchFilesIgnoreExt(
  ...
  const _byFileNoExt = (file: Vinyl) => getWithoutExt(file.relative) === getWithoutExt(fileName);
  const _byFileWithExt = (file: Vinyl) => file.relative === fileName;
  ...
  function getFile(): Vinyl | null | undefined {
    ...
    // logger.debug(
    //   `search-file-ignore-ext, ended up using ${prioritizedFile.relative} for ${fileName}`,
    // );
    return prioritizedFile;
  }

  function getMatchingFileByPriority(foundFilesWithExt: Vinyl[]): Vinyl {
