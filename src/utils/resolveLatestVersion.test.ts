...
import { getLatestVersionNumber as resolveLatestVersion } from "./resolveLatestVersion";
...
describe("getLatestVersionNumber", () => {
  ...
  const idWithVersion1WithScope = new BitId({
    scope: "my-scope",
    name: "is-string",
    version: "0.0.1",
  });
  ...
  it("should not return the id from the bitIds array if it is there with same version but different scope", () => {
    const idWithDifferentScope = new BitId({
