...
export function getWinstonLogger(logLevel: string, jsonFormat: boolean) {
...
function getMetadata(info: winston.Logform.TransformableInfo) {
  if (!Object.keys(info.metadata).length) return "";
  if (
    (info.level === "error" || info.level === "\u001b[31merror\u001b[39m") &&
    info.metadata.stack
  ) {
...
export function getFormat() {
  return winston.format.combine(
    ...
    winston.format.printf(
      (info) => `${info.timestamp} ${info.level}: ${info.message} ${getMetadata(info)}`,
    ),
  );
}
