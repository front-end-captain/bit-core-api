...
import { VersionParser } from "../component";
import { checksumFile } from "../utils";
import { IBitLogger, logger as defaultLogger } from "../logger";

export type PackResultMetadata = {
  pkgJson: Record<string, string | Record<string, string>>;
  tarPath: string;
  tarName: string;
  checksum?: string;
};
...
export class Packer {
  ...
  async npmPack(
    ...
    try {
      ...
      if (VersionParser.isSnap(pkgJson.version)) {
        warnings.push(
          `"package.json at ${cwd}" contain a snap version which is not a valid semver, can't pack it`,
        );
      ...
      const err = _err as Error & { stderr: string };
