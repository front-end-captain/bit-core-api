...
export class BitIds extends Array<BitId> {
  serialize(): string[] {
  ...
  search(bitId: BitId): BitId | null | undefined {
    return this.find(
      (id) => id.hasSameName(bitId) && id.hasSameScope(bitId) && id.hasSameVersion(bitId),
    );
  }
  ...
  toGroupByScopeName(idsWithDefaultScope: BitIds): { [scopeName: string]: BitIds } {
    return this.reduce((acc, current) => {
      ...
      if (!scopeName) {
        throw new Error(
          `toGroupByScopeName() expect ids to have a scope name, got ${current.toString()}`,
        );
      }
      if (acc[scopeName]) acc[scopeName].push(current);
      else acc[scopeName] = new BitIds(current);
      return acc;
    }, {} as Record<string, BitIds>);
  }

  findDuplicationsIgnoreVersion(): { [id: string]: BitId[] } {
    const duplications: Record<string, BitId[]> = {};
    this.forEach((id) => {
