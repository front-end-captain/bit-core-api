...
import { VersionParser, Version } from "../component/version";
...
export class BitId {
  ...
  getVersion() {
    return VersionParser.parser(this.version);
  }

  hasVersion(): boolean {
    return Boolean(this.version && this.version !== Version.LATEST_VERSION);
  }
  ...
  toString(ignoreScope = false, ignoreVersion = false): BitIdStr {
    ...
    return componentStr.concat(`${BitId.VERSION_DELIMITER}${version}`);
  }
  ...
  isVersionSnap() {
    return VersionParser.isHash(this.version);
  }
  ...
  getVersionPreReleaseData(): null | readonly string[] {
    ...
    return semver.prerelease(this.version) as string[];
  }
  ...
  static getStringWithoutVersion(id: string): string {
    const splitted = id.split(BitId.VERSION_DELIMITER);
    let res = splitted[0];
    ...
    if (id.startsWith(BitId.VERSION_DELIMITER)) {
      res = `${BitId.VERSION_DELIMITER}${splitted[1]}`;
    }
    return res;
  }

  static getVersionOnlyFromString(id: string): string {
    return id.split(BitId.VERSION_DELIMITER)[1];
  }

  static parse(id: BitIdStr, hasScope = true, version: string = Version.LATEST_VERSION): BitId {
    if (typeof id !== "string") {
      throw new TypeError(`BitId.parse expects to get "id" as a string, instead, got ${typeof id}`);
    }

    if (id.includes(BitId.VERSION_DELIMITER) && id.lastIndexOf(BitId.VERSION_DELIMITER) > 0) {
      const [newId, newVersion] = id.split(BitId.VERSION_DELIMITER);
      id = newId;
      version = newVersion;
    }
  ...
  static parseObsolete(id: BitIdStr, version: string = Version.LATEST_VERSION): BitId {
    if (id.includes(BitId.VERSION_DELIMITER)) {
      const [newId, newVersion] = id.split(BitId.VERSION_DELIMITER);
      id = newId;
      version = newVersion;
    }
  ...
  static getValidIdChunk(chunk: string): string {
    if (!isValidIdChunk(chunk)) {
      ...
      chunk = decamelize(chunk, { separator: "-" });
    }
    return chunk;
  }
  ...
  static isValidVersion(version: string): boolean {
    ...
    return BitId.isValidSemver(version) || VersionParser.isHash(version);
  }
